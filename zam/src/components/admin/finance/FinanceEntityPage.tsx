...
export default function FinanceEntityPage({
  title,
  resource,
  fields,
  columns,
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchFinanceList<Record<string, unknown>>(resource, query);
      setRows(data);
    } catch (e) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editing) await updateFinanceRecord(resource, Number(editing.id), body);
      else await createFinanceRecord(resource, body);
  ...
  const handleDelete = async (id: number) => {
    try {
      await deleteFinanceRecord(resource, id);
