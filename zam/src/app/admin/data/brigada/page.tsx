...
export default function BrigadeListPage() {
  ...
  const openCreate = () => {
    form.resetFields();
    form.setFieldsValue({ availability: 'available', status: 'active' });
    setDrawerOpen(true);
  };
  ...
  const handleCreate = async () => {
    try {
      ...
      const skills = String(values.skills_text || '')
        ...
        .map((s: string) => s.trim())
        .filter(Boolean);
      const res = await brigadaApi.create({ ...values, skills });
      if (res.success) {
        message.success('Бригад бүртгэгдлээ');
        setDrawerOpen(false);
        load();
      } else {
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Бригадын бүртгэл, hire хүсэлт, үнэлгээ, reputation
          </p>
        </div>
        <Space>
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item name="name" label="Нэр" rules={[{ required: true, message: 'Нэр заавал' }]}>
            <Input />
          </Form.Item>
          ...
          <Form.Item name="password" label="Нууц үг">
            <Input.Password placeholder="Апп нэвтрэх нууц үг" />
          </Form.Item>
          ...
          <Form.Item name="skills_text" label="Ур чадвар (таслалаар)">
            <Input.TextArea rows={2} placeholder="асфальт, бетон, гагнуур" />
          </Form.Item>
