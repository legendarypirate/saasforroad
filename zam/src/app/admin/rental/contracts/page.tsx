...
export default function RentalContractsPage() {
  ...
  const watchDaily = Form.useWatch('daily_rate', form);
  ...
  useEffect(() => {
    document.title = 'Түрээсийн гэрээ';
    fetchAll();
  }, [fetchAll]);
  ...
  const openCreate = () => {
    setEditing(null);
    form.resetFields();
    form.setFieldsValue({ status: 'active', daily_rate: 0, deposit_amount: 0 });
    setDrawerOpen(true);
  };
  ...
  const columns: ColumnsType<EquipmentRental> = [
    ...
    },
    {
      ...
      render: (_, r) => (
        <div>
          <div>{formatMnt(r.daily_rate)}/өдөр</div>
          <Text type="secondary" style={{ fontSize: 12 }}>
            ~{formatMnt(r.monthly_rate || monthlyFromDaily(r.daily_rate))}/сар
          </Text>
        </div>
      ),
    },
    ...
    <div>
      ...
      >
        <Form form={form} layout="vertical" requiredMark={false}>
          ...
          <Form.Item name="equipment_id" label="Тоног / хэрэгсэл" rules={[{ required: true, message: 'Сонгоно уу' }]}>
            <Select
              showSearch
              placeholder="Машин, шон, хүрз, арматур..."
              ...
              onChange={(id) => {
                ...
                if (eq?.default_daily_rate) {
                  form.setFieldsValue({ daily_rate: Number(eq.default_daily_rate) });
                }
              }}
            />
          </Form.Item>
          <Form.Item name="client_company" label="Түрээслэгч компани" rules={[{ required: true, message: 'Заавал' }]}>
            <Input placeholder="Замын / барилгын компанийн нэр" />
          </Form.Item>
